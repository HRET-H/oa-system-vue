...
<!-- form表单条查 -->
      ...
      <el-table :data="sealList" style="width: 100%">
        <el-table-column prop="sealName" label="用章人" width="180"></el-table-column>
        <el-table-column prop="sealTitle" label="用章名称"> </el-table-column>
        <el-table-column prop="dateOfPplication" label="申请日期"> </el-table-column>
        <el-table-column prop="state" label="状态">
          <template v-slot:default="scope">
            ...
            <el-tag v-else-if="scope.row.state == 2" type="success"
              >已同意</el-tag
            >
            <el-tag v-else-if="scope.row.state == 3" type="info"
              >已拒绝</el-tag
            >
...
// 添加职位
    ...
    deleteSeal(row) {
      axios
        .post(
          'http://localhost:9999/seal/deleteSeal?sealId=' +
            row.sealId
        )
        .then((res) => {
